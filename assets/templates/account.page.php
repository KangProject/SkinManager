...
<section>
	...
	<form action="json/" id="settingsForm" method="post" class="form-vertical shell large" autocomplete="off">
		...
		<input type="text" name="username" value="<?=$_SESSION['username']?>" placeholder="<?=Language::translate('USERNAME_ALPHANUMERIC')?>">
		...
		<select name="force2D">
			<option value="0" <?=($_SESSION['force2D']?'':'selected')?>><?=Language::translate('GRAPHMODE_SELECT_3D')?></option>
			<option value="1" <?=($_SESSION['force2D']?'selected':'')?>><?=Language::translate('GRAPHMODE_SELECT_2D')?></option>
		</select>
		<label><?=Language::translate('SWITCHSKINMODE_SELECT')?></label>
		<select name="skinmode">
			<option value="0" <?=($_SESSION['minecraft_username'] === ''?'selected':'')?>><?=Language::translate('SWITCHSKINMODE_SELECT_OFFICAL')?></option>
			<option value="1" <?=($_SESSION['minecraft_username'] === ''?'':'selected')?>><?=Language::translate('SWITCHSKINMODE_SELECT_SKINSWITCH')?></option>
		</select>
		<div <?=($_SESSION['minecraft_username'] === ''?'style="display: none;"':'')?> id="mc_login">
			<label><?=Language::translate('MINECRAFT_LOGIN')?></label>
			<input type="text" name="minecraft_username" value="<?=$_SESSION['minecraft_username']?>" placeholder="<?=Language::translate('MINECRAFT_LOGIN_TIP')?>">
			<label><?=Language::translate('MINECRAFT_PASSWORD')?></label>
			<input type="password" name="minecraft_password" value="" placeholder="<?=Language::translate('TIP_NOCHANGE')?>">
			<label><?=Language::translate('MINECRAFT_PASSWORD_KEY')?></label>
			<input type="password" name="minecraft_password_key" value="" placeholder="<?=Language::translate('TIP_NOCHANGE')?>">
		</div>
		<input type="submit">
	...
	<p><a href="unregister/" class="btn-red" style="float: right;"><?=Language::translate('UNREGISTER')?></a></p>
	<script>
