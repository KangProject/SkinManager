...
<section>
	...
	<form action="json/" method="post" class="shell" id="newSkinForm">
		...
		<input id="skinDesc" name="description" type="text" placeholder="<?=Language::translate('SKIN_DESCRIPTION')?>"/>
		...
		<label for="skinUsername"><?=Language::translate('MC_PLAYER')?> (<?=Language::translate('CASE_SENSITIVE')?>)</label>
		<input id="skinUsername" name="username" type="text" placeholder="<?=Language::translate('SKIN_LOAD_USERNAME')?>" >
		<label class="control-label" for="skinFile"><?=Language::translate('SKIN_LOAD_UPLOAD')?></label>
		<div id="skinFile"></div>
		<div class="fieldgroup-center">
			<button class="btn-blue" id="btn_submitSkin" type="submit"><?=Language::translate('SKIN_ADD')?></button>
				...
				callbacks: {
					...
					onComplete: function(id, name, response) {
						...
						if(response.error === true)
							...
							window.skinBar.addSkin({ id: response.id, title: $("#skinName").val() , description: $("#skinDesc").val() });
							window.skinBar.showSkinList();
							notificater.hide();
						}
					}
